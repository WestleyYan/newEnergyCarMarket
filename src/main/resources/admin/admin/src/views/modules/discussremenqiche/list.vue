		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0px 0px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('discussremenqiche','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","borderRadius":"5px","height":"34px"}'></span>
						ADD
					</el-button>
					<el-button class="del" v-if="isAuth('discussremenqiche','DELETE')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"15px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="Actions">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discussremenqiche','UPDATE') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								UPDATE
							</el-button>

							<el-button class="view" v-if="isAuth('discussremenqiche','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
	...
	export default {
		...
		methods: {
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussdiscussremenqiche',query:{refid:id}});
			},
			backClick(){
				history.back()
			},
			...
			async deleteHandler(id ,refid ) {
				...
				await this.$confirm(`确定进行[${id ? "DELETE" : "批量DELETE"}]Actions?`, "提示", {
					...
					await this.$http({
						url: "discussremenqiche/delete",
						method: "post",
						data: ids
					}).then(async ({ data }) => {
						if (data && data.code === 0) {
							this.$http({
								url: 'remenqiche/info/' + refid,
								method: "get",
							}).then(res=>{
