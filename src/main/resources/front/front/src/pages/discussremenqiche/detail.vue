	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'——'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/discussremenqiche?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('discussremenqiche','UPDATE')" @click="editClick">UPDATE</el-button>
						<el-button class="delBtn" v-if="btnAuth('discussremenqiche','DELETE')" @click="delClick">DELETE</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/discussremenqiche', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page) {
				this.$http.get('discussdiscussremenqiche/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussdiscussremenqiche/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否DELETE此评论？').then(_ => {
					this.$http.post('discussdiscussremenqiche/delete',[id]).then(res=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$http.post('discussdiscussremenqiche/add', this.form).then(rs2 => {
							if (rs2.data.code == 0) {
								this.form.content = '';
			...
			addDiscussNum(type){
				...
				this.$http.post('discussremenqiche/update',this.detail).then(res=>{})
			},
			...
			// UPDATE
			editClick(){
				this.$router.push(`/index/discussremenqicheAdd?type=edit&&id=${this.detail.id}`);
			},
			// DELETE
			async delClick(){
				await this.$confirm('是否DELETE此Popular Cars评论表？') .then(_ => {
					this.$http.post('discussremenqiche/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: 'DELETE成功!',
