	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'——'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/remenqiche?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
	...
	export default {
		...
		methods: {
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('remenqiche/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'remenqiche', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/remenqiche', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page) {
				this.$http.get('discussremenqiche/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussremenqiche/update',row).then(rs=>{
					...
					if (valid) {
						this.$http.get('orders/list', {params: {page: 1, limit: 1, status: '已完成', goodid: this.detail.id, userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
							if (res.data.code == 0 && res.data.data.list.length == 0) {
								...
								this.$http.get('discussremenqiche/page',{params: {
									page: 1,
									limit: 1,
									...
									this.$http.post('discussremenqiche/add', this.form).then(rs2 => {
										if (rs2.data.code == 0) {
			...
			addDiscussNum(type){
				...
				this.$http.get('discussremenqiche/list',{params: {
					page: 1,
					limit: 10000,
					refid: this.detail.id
				}}).then(rs=>{
			...
			getCartList() {
				this.$http.get('cart/list', {params: {userid: Number(localStorage.getItem('frontUserid')), tablename: 'remenqiche', goodid: this.detail.id}}).then(res => {
					if (res.data.code == 0) {
