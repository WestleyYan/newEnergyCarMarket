	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'——'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/discussxinnengyuanqiche?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">赞Userids</div>
						<div class="text "  >{{detail.tuserids}}</div>
					</div>
					<div class="item">
						<div class="lable">踩Userids</div>
						<div class="text "  >{{detail.cuserids}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('discussxinnengyuanqiche','UPDATE')" @click="editClick">UPDATE</el-button>
						<el-button class="delBtn" v-if="btnAuth('discussxinnengyuanqiche','DELETE')" @click="delClick">DELETE</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/discussxinnengyuanqiche', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page) {
				this.$http.get('discussdiscussxinnengyuanqiche/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussdiscussxinnengyuanqiche/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否DELETE此评论？').then(_ => {
					this.$http.post('discussdiscussxinnengyuanqiche/delete',[id]).then(res=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$http.post('discussdiscussxinnengyuanqiche/add', this.form).then(rs2 => {
							if (rs2.data.code == 0) {
								this.form.content = '';
			...
			addDiscussNum(type){
				...
				this.$http.post('discussxinnengyuanqiche/update',this.detail).then(res=>{})
			},
			...
			// UPDATE
			editClick(){
				this.$router.push(`/index/discussxinnengyuanqicheAdd?type=edit&&id=${this.detail.id}`);
			},
			// DELETE
			async delClick(){
				await this.$confirm('是否DELETE此New Energy Vehicles评论表？') .then(_ => {
					this.$http.post('discussxinnengyuanqiche/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: 'DELETE成功!',
