<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="User名" prop="nickname">
				<el-input v-model="ruleForm.nickname" 
					placeholder="User名" clearable :disabled=" false  ||ro.nickname"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="Rating" prop="score">
				<el-input-number v-model="ruleForm.score" placeholder="Rating" :disabled=" false ||ro.score"></el-input-number>
			</el-form-item>
			...
			<el-form-item class="add-item" label="回复内容" prop="reply">
				<el-input
					type="textarea"
					:rows="8"
					:disabled=" false  ||ro.reply"
					placeholder="回复内容"
					v-model="ruleForm.reply">
					</el-input>
			</el-form-item>
			<el-form-item class="add-item" label="赞Userids" prop="tuserids">
				<el-input
					type="textarea"
					:rows="8"
					:disabled=" false  ||ro.tuserids"
					placeholder="赞Userids"
					v-model="ruleForm.tuserids">
					</el-input>
			</el-form-item>
			<el-form-item class="add-item" label="踩Userids" prop="cuserids">
				<el-input
					type="textarea"
					:rows="8"
					:disabled=" false  ||ro.cuserids"
					placeholder="踩Userids"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`discussxinnengyuanqiche/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`discussxinnengyuanqiche/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: 'Actions成功',
									type: 'success',
