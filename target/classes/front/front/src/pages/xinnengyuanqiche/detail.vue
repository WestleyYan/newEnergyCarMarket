	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'——'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/xinnengyuanqiche?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">Unit Limit</div>
					...
					<div class="item">
						<div class="lable">Car Categories</div>
						<div class="text "  >{{detail.qichefenlei}}</div>
					</div>
					<div class="item">
						<div class="lable">Specification</div>
						<div class="text "  >{{detail.guige}}</div>
					</div>
					<div class="item">
						<div class="lable">Features</div>
						<div class="text "  >{{detail.tedian}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						...
						<el-button class="addCartBtn" v-if="detail.alllimittimes" type="warning" size="small" @click="addCart">ADD到购物车</el-button>
						<el-button class="buyBtn" v-if="detail.alllimittimes" type="warning" size="small" @click="buyNow">立即购买</el-button>

					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('xinnengyuanqiche','UPDATE')" @click="editClick">UPDATE</el-button>
						<el-button class="delBtn" v-if="btnAuth('xinnengyuanqiche','DELATE')" @click="delClick">DELATE</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="汽车参数" name="first">
					<div class="ql-snow ql-editor" v-html="detail.qichecanshu"></div>
				</el-tab-pane>
				<el-tab-pane label="评论" name="second">
					<el-form class="add commentForm" :model="form" :rules="rules" ref="form">
						...
						<el-form-item class="item" label="Rating" prop="score">
							<el-rate
								v-model="form.score"
								:max='Number(5)'
								:allow-half='false'
	...
	export default {
		...
		data() {
			return {
				...
				rules: {
					...
					score: [
						{ required: true, message: '请选择Rating', trigger: 'blur' }
					],
				},
				total: 1,
				pageSize: 10,
		...
		methods: {
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('xinnengyuanqiche/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'xinnengyuanqiche', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			async thumbsupOrCrazily(type) {
				...
				await this.$http.post('storeup/add', this.storeupParams).then(res => {
					if (res.data.code == 0) {
						...
						this.$http.post('xinnengyuanqiche/update', detail).then(res => {
							this.detail = detail
						});
						this.getThumbsupOrCrazilyStatus();
						this.$message({
			...
			getThumbsupOrCrazilyStatus() {
				if(localStorage.getItem("frontToken")) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '21', refid: this.detail.id, tablename: 'xinnengyuanqiche', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
							this.isThumbsupnum = true;
							this.thumbsupOrCrazilyInfo = res.data.data.list[0];
						}
					});
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '22', refid: this.detail.id, tablename: 'xinnengyuanqiche', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/xinnengyuanqiche', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page) {
				this.$http.get('discussxinnengyuanqiche/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussxinnengyuanqiche/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否DELATE此评论？').then(_ => {
					this.$http.post('discussxinnengyuanqiche/delete',[id]).then(res=>{
					...
					if (valid) {
						this.$http.get('orders/list', {params: {page: 1, limit: 1, status: '已完成', goodid: this.detail.id, userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
							if (res.data.code == 0 && res.data.data.list.length == 0) {
								...
								this.$http.get('discussxinnengyuanqiche/page',{params: {
									page: 1,
									limit: 1,
									...
									this.$http.post('discussxinnengyuanqiche/add', this.form).then(rs2 => {
										if (rs2.data.code == 0) {
			...
			addDiscussNum(type){
				...
				this.$http.get('discussxinnengyuanqiche/list',{params: {
					page: 1,
					limit: 10000,
					refid: this.detail.id
				}}).then(rs=>{
			...
			getCartList() {
				this.$http.get('cart/list', {params: {userid: Number(localStorage.getItem('frontUserid')), tablename: 'xinnengyuanqiche', goodid: this.detail.id}}).then(res => {
					if (res.data.code == 0) {
			...
			addCart() {
				...
				if (this.detail.alllimittimes <= 0 ) {
					this.$message.error(`Product已售罄`);
					return
				}
				// Stock限制
				if (this.detail.alllimittimes > 0 && this.detail.alllimittimes < this.buynumber) {
					this.$message.error(`Stock不足`);
					return
				}
				if (this.isInCart) {
					this.$message.error('该Product已经ADD到购物车');
					return;
				}
			...
			buyNow() {
				...
				// 保存到storage中，在确认下单页面中获取要购买的Product
			...
			// UPDATE
			editClick(){
				this.$router.push(`/index/xinnengyuanqicheAdd?type=edit&&id=${this.detail.id}`);
			},
			// DELATE
			async delClick(){
				await this.$confirm('是否DELATE此New Energy Vehicles？') .then(_ => {
					this.$http.post('xinnengyuanqiche/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
