		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0px 0px"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","padding":"10px","flexWrap":"wrap","background":"none","display":"flex"}' >
					<div :style='{"margin":"0 10px 20px 0","display":"flex"}'>
						<label :style='{"border":"1px solid #CBCBCB","padding":"0 10px","margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#000","textAlign":"center","display":"inline-block","borderRadius":"10px","background":"#fff","width":"auto","lineHeight":"34px","fontSize":"inherit","fontWeight":"500","height":"34px"}' class="item-label">User名</label>
						<el-input v-model="searchForm.nickname" placeholder="User名" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 20px 0","display":"flex"}'>
						<label :style='{"border":"1px solid #CBCBCB","padding":"0 10px","margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#000","textAlign":"center","display":"inline-block","borderRadius":"10px","background":"#fff","width":"auto","lineHeight":"34px","fontSize":"inherit","fontWeight":"500","height":"34px"}' class="item-label">评论内容</label>
				...
				<el-row :style='{"width":"100%","padding":"10px","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","color":"#0F1330","borderRadius":"5px","background":"#AFD0F5","width":"auto","fontSize":"16px","height":"34px"}' type="success" @click="backClick()">返回</el-button>
				</el-row>

				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('discussxinnengyuanqiche','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","borderRadius":"5px","height":"34px"}'></span>
						ADD
					</el-button>
					<el-button class="del" v-if="isAuth('discussxinnengyuanqiche','DELATE')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"15px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="score" label="Rating">
						<template slot-scope="scope">
							<el-rate
					...
					<el-table-column width="300" label="Actions">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discussxinnengyuanqiche','UPDATE') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								UPDATE
							</el-button>

							<el-button class="view" v-if="isAuth('discussxinnengyuanqiche','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				this.$http({
					url: "discussxinnengyuanqiche/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussdiscussxinnengyuanqiche',query:{refid:id}});
			},
			backClick(){
				history.back()
			},
			discussistopChange(e,row){
				this.$http({
					url:'discussxinnengyuanqiche/update',
					method:'post',
					data:row
				}).then(res=>{})
			},
			...
			async deleteHandler(id ,refid ) {
				...
				await this.$confirm(`确定进行[${id ? "DELATE" : "批量DELATE"}]Actions?`, "提示", {
					...
					await this.$http({
						url: "discussxinnengyuanqiche/delete",
						method: "post",
						data: ids
					}).then(async ({ data }) => {
						if (data && data.code === 0) {
							this.$http({
								url: 'xinnengyuanqiche/info/' + refid,
								method: "get",
							}).then(res=>{
